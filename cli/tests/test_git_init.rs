...
fn test_git_init_colocated_import_branches() {
    ...
    let workspace_root = test_env.env_root().join("repo_unable_to_track");
    ...
    Done importing changes from the underlying Git repo.
    Setting the revset alias "trunk()" to "my-bookmark@origin"
    Hint: The following remote bookmarks were not automatically tracked:
      my-bookmark@origin
    Hint: Run `jj bookmark track my-bookmark@origin` to keep local bookmarks updated on future pulls.
    Initialized repo in "repo_unable_to_track"
    "#);

    // Check that the bookmark is *not* tracked, since the local and remote
